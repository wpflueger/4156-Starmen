...
export const Calendar = (props: ProfileSubpageProps) => {
    ...
    const [patient, setPatient] = useState(props.patients.length > 0 ? props.patients[0]: undefined as Patient | undefined)
	...
	useEffect(() => {
        ...
        setScheduleError('')
		setIsValid(
			subject.length > 0 && Validator.text(subject) &&
    ...
    const scheduleAppointment = async () => {
        ...
        try {
            ...
            setScheduleError(`Looks like ${props.isPatient ? doctor!.firstName : patient!.firstName} is busy during this time.`)
        }
		...
		<div className={styles.calendar_outter}>
            <div className={styles.calendar_top}>
                <div className={styles.calendar_bar}>
                    <div>
                        <b>Today: </b>{DateTime.prettyDate(new Date())}
                    </div>
                    <Button
                    ...
                    <div className={`${styles.popup_mid_inputs} d-flex flex-direction-row mt-3 mb-4`}>
                        <div className={styles.input_short}>
                            ...
                            }
                            {
                                ...
                                <Dropdown
                                    displayOptions={newPatients.map(p => `${p.lastName}, ${p.firstName} <${p.email}>`)}
                            ...
                            {
                                scheduleError &&
                                <div className={'mt-4 h6 color-quaternary'}>
                                    {scheduleError}
                                </div>
                            }
                        </div>
