...
export const HoursInputs = (props: HoursInputsProps) => {
    ...
    return (
        <div
            className={`${styles.hours_outter} ${props.className ? props.className : ''}`}
        >
            {props.label && (
                <div>
                    {props.label}
            ...
            )}
            {
                DAYS.map((day, idx) => (
                    ...
                    >
                        <div
                            className={'col-1 d-flex flex-direction-row justify-content-start p-0 unselectable clickable'}
                        >
                            <input type="checkbox" name={`${name}-${day}`} value={day} onChange={() => onDaySelect(day)} checked={getHours(day).startTime !== TIME_ABSENT} />
                        </div>
                        <div
                            className={`col-3 p-0 ${getHours(day).startTime !== TIME_ABSENT ? 'font-weight-bold' : ''} unselectable clickable`}
                        >
                            <label
                                htmlFor={`${name}-${day}`}
                                className={'clickable'}
