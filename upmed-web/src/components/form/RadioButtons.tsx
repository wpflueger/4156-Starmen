...
export const RadioButtons = (props: RadioButtonsProps) => {
    const name = `radio-${new Date().getTime()}-${Hasher.encode(props.options.join(','))}-${Hasher.encode(props.label || Math.random().toString())}`
    ...
    return (
        <div
            className={`${styles.radio_outter} ${props.className || ''}`}
        >
            {props.label && (
                <div>
                    {props.label}
